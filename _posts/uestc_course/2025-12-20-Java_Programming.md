...
* 类的构造函数设为private会阻止其他类继承
* 在调用内部类时使用 new Other().new Inner()时，会触发Other的构造函数
* 每个非静态内部类实例都隐式持有一个指向其外部类实例的引用，只要内部类实例存在，它所持有的外部类实例就不会被垃圾回收
* java中只有内部类可以是静态的，对于一个静态类内部，里面的成员变量能不是静态的。
* 为了确保“里氏替换原则”和多态性，子类对象必须能够替换父类对象，因此访问权限不可更严格
* 方法重写必要条件

| 触发条件 | 必须满足 | 否则 |
|:--------|:---------|:------------|
| **1. 继承关系** | 子类继承父类 | 无继承，无重写 |
| **2. 方法签名相同** | 方法名、参数列表完全相同 | 这是重载，不是重写 |
| **3. 返回类型兼容** | 相同或是其子类（协变返回） | 编译错误 |
| **4. 访问权限不更严格** | 子类方法权限 ≥ 父类 | 编译错误 |
| **5. 异常抛出兼容** | 子类异常 ≤ 父类异常 | 编译错误 |

* 实例多态时的成员性质

| 成员类型 | 调用哪个版本 | 是否有多态 |
|:---|:---|:---|
| **实例方法**（非静态、非final、非private） | 子类的版本 ✅ | ✅ 有多态（动态绑定） |
| **静态方法** | 父类的版本 ❌ | ❌ 无多态（编译时绑定） |
| **静态变量** | 父类的版本 ❌ | ❌ 无多态（编译时绑定） |
| **实例变量**（字段） | 父类的版本 ❌ | ❌ 无多态（编译时绑定） |
...
* 接口和抽象类的区别点
![alt text](/img/in-post/Java_Programming/Differences_between_interfaces_and_abstract_classes.png)
...
* package使用注意要点：
  ...
  * package语句最多只有一句。
* Java的核心类都放在java包及其子包下，Java扩展的类都放在javax包及其子包下，如下表所示。
![alt text](/img/in-post/Java_Programming/Core_classes_of_Java.png)
* 在try-catch中执行System.exit(0)语句，可以强制不执行finally块中的代码
* throws声明的方法表示此方法不处理异常，而交给方法的调用处进行处理。因此，不管方法是否发生异常，调用者都必须进行异常处理，要么继续throws，要么try-catch，不然会导致编译错误。
* 实例变量、实例方法与类变量、类方法

| 概念 | 也称作 | 说明 |
| --- | --- | --- |
| 实例变量 | 成员变量 / 属性 / 字段 | 对象的状态。Java 官方称“字段”，日常交流多用“属性”或“成员变量”。 |
| 类变量 | 静态变量 | 用 `static` 修饰，属于类而非对象。 |
| 实例方法 | 成员方法 / 非静态方法 | 属于对象的方法。“非静态”是其最直接的技术反义词。 |
| 类方法 | 静态方法 | 属于类的方法，这是最常用的别名。 |

* catch子句必须按照从特殊到一般的顺序排列，即子类异常在先，父类异常在后，否则会导致编译错误。
