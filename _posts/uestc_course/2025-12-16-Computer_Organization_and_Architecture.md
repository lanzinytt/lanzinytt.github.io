...
1. 流水线数据通路和控制
    ...
    1. 有利于流水线执行的指令集特征（四个特征）：
    - 指令长度固定，便于译码和分段。
    - 寻址方式简单，减少译码和执行复杂度。
    - 操作数位置规则，便于硬件实现流水线。
    - 指令执行时间均衡，避免流水段负载不均。

    1. 常见7条指令的各个流水阶段划分及功能部件分析：
...
1. 存储器概述和存储器芯片
    ...
    1. **各类存储器的含义**：
        ...
        - 顺序存取存储器（SAM）：数据按固定顺序读写，如磁带。
        - 直接存取存储器（DAM）：可按地址直接访问数据，但寻址时间与位置有关，如磁盘。
    ...
    3. 静态存储器和动态存储器的工作机制：
        ...
        - 动态存储器的刷新分为**按行刷新、集中刷新、分散刷新和异步刷新**。
...
1. I/O系统及I/O设备
    ...
    3. 磁盘上数据定位采用**磁道号、磁头号（盘面号）、扇区号**三元组。磁盘数据的存取以块（扇区）为单位。磁盘操作包括寻道操作（移动磁头到指定磁道）、旋转等待操作（等待目标扇区转到磁头下方）和读写操作（实际数据传输）。
...
2. 总线及系统互连、I/O接口
    ...
    3. I/O端口有**独立编址和统一编址**两种方式。独立编址将I/O端口与内存地址空间分开，统一编址将I/O端口映射到内存地址空间，便于统一管理和访问。
...
3. **`I/O数据传送控制方式`**
    ...
    5. DMA控制器负责管理DMA传输，包括地址计数、数据缓冲和中断请求。DMA操作分为**启动、传输和结束（软硬软）**三个步骤。DMA I/O传送方式与中断I/O传输方式相比，DMA方式CPU干预少，效率更高，适合大批量数据传输。
